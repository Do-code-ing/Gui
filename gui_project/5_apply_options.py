...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\pc\Desktop\PythonWorkspace\pygame_project\images")
        # filedialog.askopenfilenames() = 파일 탐색기를 열고 파일 여러개를 한 번에 가져올 수 있음
        # initialdir = 사용자가 지정한 경로를 보여줌
    for file in files:
        list_file.insert(END, file)

# 선택 삭제
def del_file():
    for index in reversed(list_file.curselection()): # curselection = current selection = 지금 선택된 것
...
def merge_image():
    ...
    try:
        ...
        # 최대 넓이, 전체 높이 값 구하기
        max_width, total_height = max(widths), sum(heights) # max는 입력 값 중 최대 값만을, sum은 모든 값을 합쳐서 반환
        
        # 스케치북 준비
        if img_space > 0: # 이미지 간격 옵션 적용
            total_height += (img_space * len(images) - 1) # -1 하는 이유는 0부터 len으로 세기 때문
